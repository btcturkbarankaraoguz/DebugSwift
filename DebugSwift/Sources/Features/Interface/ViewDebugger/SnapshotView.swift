...
private func highlightNode(snapshot: Snapshot, color: UIColor) -> SCNNode {
    ...
    node.position = SCNVector3(x: .zero, y: .zero, z: smallZOffset)
    return node
}
...
private func lineFrom(vertex vertex1: SCNVector3, toVertex vertex2: SCNVector3, color: UIColor) -> SCNNode {
    ...
    material.diffuse.contents = color
    material.isDoubleSided = true
    geometry.insertMaterial(material, at: .zero)
...
private func headerNode(
    ...
    let frame = CGRect(x: .zero, y: .zero, width: headerWidth, height: CGFloat(textHeight) + (attributes.verticalInset * 2.0))
